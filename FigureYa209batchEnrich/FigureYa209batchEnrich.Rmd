...
gmt <- msigdbr::msigdbr(species = target_species) 
...
# 检查所需的列是否存在
required_cols <- c("gs_name", "entrez_gene", "gs_cat")
missing_cols <- setdiff(required_cols, colnames(gmt))

if (length(missing_cols) > 0) {
  cat("以下列不存在:", paste(missing_cols, collapse = ", "), "\n")
  cat("可用的列:", paste(colnames(gmt), collapse = ", "), "\n")
  
  # 尝试找到可能的列名变体
  possible_gs_name <- grep("gs.*name", colnames(gmt), ignore.case = TRUE, value = TRUE)
  possible_entrez <- grep("entrez", colnames(gmt), ignore.case = TRUE, value = TRUE)
  possible_gs_cat <- grep("gs.*cat", colnames(gmt), ignore.case = TRUE, value = TRUE)
  
  cat("可能的gs_name列:", ifelse(length(possible_gs_name) > 0, possible_gs_name[1], "未找到"), "\n")
  cat("可能的entrez_gene列:", ifelse(length(possible_entrez) > 0, possible_entrez[1], "未找到"), "\n")
  cat("可能的gs_cat列:", ifelse(length(possible_gs_cat) > 0, possible_gs_cat[1], "未找到"), "\n")
  
  # 使用找到的列名或停止执行
  gs_name_col <- ifelse(length(possible_gs_name) > 0, possible_gs_name[1], stop("找不到gs_name列"))
  entrez_col <- ifelse(length(possible_entrez) > 0, possible_entrez[1], stop("找不到entrez_gene列"))
  gs_cat_col <- ifelse(length(possible_gs_cat) > 0, possible_gs_cat[1], stop("找不到gs_cat列"))
  
} else {
  gs_name_col <- "gs_name"
  entrez_col <- "entrez_gene"
  gs_cat_col <- "gs_cat"
}

# 选择需要的列并分组（使用安全的列选择方式）
