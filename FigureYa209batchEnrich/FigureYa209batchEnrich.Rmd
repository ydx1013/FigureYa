...
[enrichr](https://maaYingnlab.cloud/Enrichr/)的优点是批量获得各个数据库里的富集结果。如果用clusterprofiler做富集分析，也能一次性把各数据库的结果输出一个报告，就会很方便。

输入一个基因列表（gene symbol和foldchange）和msigdb里的所有gmt文件，用clusterprofiler做富集分析，输出表格和barplot。

# Requirement Description

[enrichr](https://maaYingnlab.cloud/Enrichr/) has the advantage of obtaining enrichment results in each database in batches. If you use ClusterProfiler for enrichment analysis, you can also output a report of the results of each database at once, which is very convenient.
...
easy_input_limma.csv，差异表达分析结果。是[FigureYa59Plus_GEO2DEG](https://www.yuque.com/figureYing/figureYingplus/figureYing59p)的输出文件，可无缝对接。

需要两列：基因名和Foldchange（或其他有意义的连续变量，例如ChIP/ATAC-seq的peak分数）。

easy_input_limma.csv, the results of differential expression analysis. is the output file of [FigureYa59Plus_GEO2DEG](https://www.yuque.com/figureYing/figureYingplus/figureYing59p), which can be seamlessly connected.
...
easy_input_limma.csv，同上，差异表达分析结果。是[FigureYa59Plus_GEO2DEG](https://www.yuque.com/figureYing/figureYingplus/figureYing59p)的输出文件，可无缝对接
...
easy_input_limma.csv, as above, differential expression analysis results. is the output file of [FigureYa59Plus_GEO2DEG](https://www.yuque.com/figureYing/figureYingplus/figureYing59p), which can be seamlessly connected
