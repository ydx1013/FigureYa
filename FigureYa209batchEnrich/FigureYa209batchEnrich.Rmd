...
# 查看支持的物种 / View supported species
supported_species <- msigdbr::msigdbr_species()
print(supported_species)

# 获取人类基因集 / Get human gene sets
target_species <- "Homo sapiens"  # 可修改为其他物种 / Can be modified to other species
gmt <- msigdbr::msigdbr(species = target_species) 

# 选择需要的列并分组 / Select required columns and group
gmt2 <- gmt %>%
  dplyr::select(gs_name, entrez_gene)  # 基因集名称和Entrez ID / Gene set name and Entrez ID

gmts <- split(gmt2, gmt$gs_cat)  # 按基因集类别分组 / Group by gene set category
