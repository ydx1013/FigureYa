...
作者用Bland-Altman的方法比较了自己建立的模型和传统的模型的一致性。网上找了下，好像没有可以重复这个的代码，求实现。
The author compared the consistency between the model he established and the traditional model using the Bland-Altman method. I searched online and it seems there's no code that can repeat this. I'm seeking crowdfunding.
...
The Bland-Altman plot was used to compare the agreements among different evaluations and to conduct consistency (slope) tests.
...
自定义函数绘制BAplot并做斜率检验
A custom function is used to draw BAplot and perform slope tests
...
easy_input.csv, the first column is error and the second column is grouping information. Here are the test results and the gold standard results.
...
By calling the previous custom function, verification and drawing can be accomplished in one step.
