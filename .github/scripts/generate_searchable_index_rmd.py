...
def get_rmd_preview_content(rmd_path):
    ...
    try:
        ...
        # 合并注释和代码块预览
        preview_content = '\n'.join(preview_lines[:10])  # 前10行注释
        if code_blocks:
            preview_content += '\n\n' + '\n\n'.join(code_blocks[:3])  # 前3个代码块
        
        return title, description, preview_content[:1000]  # 限制预览长度
        ...
        if target_files:
            ...
            for fname in target_files:
                ...
                chapters_meta.append({
                    ...
                    "folder": folder,
                    "thumb": thumb_path,
                    "description": description if file_type == "rmd" else ""
                })
...
def create_rmd_viewer_page(folder, filename, rmd_path):
    ...
    viewer_path = os.path.join(PUBLISH_DIR, "rmd_viewers", f"{folder}_{safe_filename}.html")
...
<head>
    ...
    <style>
        body {{ font-family: 'Monaco', 'Menlo', 'Ubuntu Mono', monospace; font-size: 14px; line-height: 1.5; }}
        .container {{ max-width: 1200px; margin: 0 auto; padding: 20px; }}
        .header {{ background: #f5f5f5; padding: 15px; border-radius: 5px; margin-bottom: 20px; }}
        .code-container {{ background: #f8f8f8; border: 1px solid #ddd; border-radius: 5px; padding: 15px; }}
        pre {{ margin: 0; white-space: pre-wrap; word-wrap: break-word; }}
        .yaml-header {{ color: #22863a; font-weight: bold; }}
        .comment {{ color: #6a737d; }}
        .code-block {{ color: #032f62; }}
...
<body>
    <div class="container">
        <div class="back-btn">
            <button onclick="window.history.back()">← Back to Browser</button>
            <button onclick="window.location.href='../../index.html'">← Back to Home</button>
        </div>
        
        <div class="header">
            <h1>{filename}</h1>
            <p>Folder: {folder} | File: {filename}</p>
            <p>
                <a href="../../{rmd_path}" download>Download RMD</a> | 
...
# 生成index.html（修改JavaScript部分以处理Rmd文件在线查看）
html_output = """
<!DOCTYPE html>
<html>
<head>
