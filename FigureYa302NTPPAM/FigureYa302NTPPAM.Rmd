...
source("CMScaller.R") # From CMScaller package
source("replaceGeneId.R") # From CMScaller package
library(GEOquery)
library(pamr)
library(cluster)
...
gse17536.expr.txt - GEO结直肠癌表达谱文件。

gse17536.expr.txt – A GEO colorectal cancer expression profile file.

```{r}
# 下载GSE17536数据集
# Download GSE17536 dataset
gse <- getGEO(GEO = "GSE17536", destdir = ".", getGPL = FALSE)
...
# 对行名进行标准化，重复基因取均值（如果需要）
# Standardize row names - take mean for duplicate genes (if needed)
# 注意：GEO数据集通常已经处理好了行名，可能不需要这一步
# Note: GEO datasets usually have pre-processed row names, this step may not be necessary
if(any(duplicated(rownames(expr)))) {
  expr$gene <- rownames(expr)
  expr <- apply(expr[, setdiff(colnames(expr), "gene")], 2, 
                function(x) tapply(x, INDEX = factor(expr$gene), FUN = mean, na.rm = TRUE))
  expr <- as.data.frame(expr)
}
```
