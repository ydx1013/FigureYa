...
```{r}
# 加载 STEP 1 获得的pbmc # Load the PBMC file obtained in Step 1
# Load the PBMC file obtained in Step 1
...
# Import the gmt file. Here, we use the Hallmark file from MsigDB as an example.
# MsigDB link (https://www.gsea-msigdb.org/gsea/msigdb/)
# When downloading the file, be sure to select the corresponding gene format. Generally, 10X results are in gene symbol format.
genesets <- getGmt("h.all.v7.0.symbols.gmt")

### 1. 找到感兴趣的细胞组别，两者间进行比较
# 这里以cluster6和cluster8为例
### 1. Find the cell groups of interest and compare them.
# Here, we use cluster 6 and cluster 8 as examples.
subcluster = c(6, 8)
sub <- subset(pbmc, idents = subcluster)
df.data <- GetAssayData(object = sub, slot = "data")

# 将细胞所属的group信息保存起来
# Save the cell group information.
...
### 2. Perform gsva analysis.
# Since the data is normalized, the default parameters are sufficient. Adjust parallel.sz according to your computer's CPU.
gsvascore <- gsva(data.matrix(df.data), genesets, parallel.sz = 2)
# Because many gene expression values in the single-cell data are 0, a red warning message will appear. This is not a problem.
...
# First use a heatmap to display the results for all 50 genesets in hallmarker
...
### 3. 挑选出要使用小提琴图展示的通路
### 3. Select pathways to display using violin plots
# 使用limma包进行差异分析，然后根据差异分析结果，对通路进行选择。
# Use the limma package to perform differential analysis and then select pathways based on the results.
cluster.group <- as.numeric(df.group$cluster) 
# 确定分组信息
# Determine grouping information
group2 <- 6 
# 根据子集中细胞类型的名称来定
# Determine based on the cell type name in the subset
# 这里cluster6作为实验组（group2)，cluster8作为对照组（group1)
# Here, cluster6 is the experimental group (group2), and cluster8 is the control group (group1)
...
# 利用limma包进行差异分析
# Perform differential analysis using the limma package
...
# Add grouping information to facilitate distinguishing experimental and control groups in the exported data.
