...
p_top <- ggplot(df, aes(x = AUC, color = Subtype, fill = Subtype)) +
  ...
  scale_color_manual(values = c(alpha(green,0.7),alpha(cYingn,0.7),alpha(blue,0.7))) + # 设置透明色 # Set the transparent color
  scale_fill_manual(values = c(alpha(green,0.7),alpha(cYingn,0.7),alpha(blue,0.7))) +
...
p_bot <- ggplot(df, aes(Subtype, AUC, fill = Subtype)) + 
  ...
  scale_fill_manual(values = c(green, cYingn, blue)) + 
  scale_color_manual(values = c(green, cYingn, blue)) + 
