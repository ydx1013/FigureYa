...
```{r}
# 解压文件并读取
unzipped_files <- unzip('tcga_blca_fpkm.txt.zip', exdir = tempdir())
data_file <- unzipped_files[1]  # 取第一个文件

# 读取数据
expr <- fread(data_file, sep = "\t", header = TRUE, check.names = FALSE)
...
immPath.score <- GSVA::gsva(
  expr = as.matrix(log2(expr[,tumsam] + 1)),
  gset.idx.list = immPath.list,  # 明确指定参数名 / Explicitly specify parameter name
  method = "ssgsea"
)
...
blcaPath.score <- GSVA::gsva(
  expr = as.matrix(log2(expr[,tumsam] + 1)),
  gset.idx.list = blcaPath.list,  # 明确指定参数名 / Explicitly specify parameter name
  method = "ssgsea"
)
# 保存到文件，便于套用 / Save to file for easy reuse
...
gset.score <- GSVA::gsva(
  expr = as.matrix(log2(expr[,tumsam] + 1)),
  gset.idx.list = gset.list,  # 明确指定参数名 / Explicitly specify parameter name
  method = "ssgsea"
)
# 保存到文件 / Save to file
