...
# from estimate package, https://rdrr.io/rforge/estimate/f/
filterCommonGenes <- function(input.f,
    ...
    ## Read input data: accept either file name or data.frame
    if (is.character(input.f) && length(input.f) == 1 && nzchar(input.f)) {
    ...
    } else if (is.data.frame(input.f)) {
        input.df <- input.f
    } else {
        stop("input.f must be either a file name or a data.frame")
    }
    
    ## Check output.f argument
...
load("common_genes.rda") # from tidyestimate package, https://github.com/KaiAragaki/tidyestimate/blob/main/data/common_genes.rda

filterCommonGenes(input.f = indata , output.f="TCGA_log2TPM_hugo_ESTIMATE.txt", id="GeneSymbol")
