...
gse <- getGEO(GEO = "GSE103322", destdir = ".", getGPL = FALSE)

# Extract expression matrix
exprdata <- exprs(gse[[1]])

# Extract cell annotation information (phenotype data)
annodata <- pData(gse[[1]])

# Check data structure
dim(exprdata)
dim(annodata)
head(colnames(annodata))

# Process tumor ID information (following the logic of the original code)
# Assuming cell IDs are in the row names of annodata
tmp <- reshape2::colsplit(
  gsub(pattern = "HNSCC_17", replacement = "HNSCC17", x = rownames(annodata)),
  "_", names = letters[1:10]
)
annodata$tumor <- gsub(pattern = "HN|HNSCC", replacement = "MEEI", tmp$a, ignore.case = TRUE)
