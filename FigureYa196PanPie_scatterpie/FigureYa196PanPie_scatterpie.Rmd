---
title: "FigureYa196PanPie_scatterpie.html"
params:
  author: Ying Ge
...
stage.col <- c("#EFF3FF", "#BDD7E7", "#6BAED6", "#2171B5")  # 蓝色渐变
N.col <- c("#E5F5E0", "#A1D99B", "#31A354")  # 绿色渐变
T.col <- c("#F2F0F7", "#CBC9E2", "#9E9AC8", "#6A51A3")  # 紫色渐变

# 修正的prepare_pie_data函数
prepare_pie_data <- function(pie.list, group) {
  ...
  for (i in 1:length(pie.list)) {
    ...
    for (j in 1:nrow(pie_df)) {
      level <- as.character(pie_df$Var2[j])  # 确保转换为字符
      pct <- pie_df$Pct[j]
