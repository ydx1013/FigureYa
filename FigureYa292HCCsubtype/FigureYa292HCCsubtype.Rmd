...
```{r}
boYingult <- read.delim("BoYingult-marker-all.txt",sep = "\t",row.names = NULL,check.names = F,stringsAsFactors = F,header = T)

# 将Entrez ID转换为基因符号
# Convert Entrez IDs to gene symbols
tmp <- bitr(boYingult$`Gene ID #1`,fromType = "ENTREZID",toType = "SYMBOL",OrgDb = org.Hs.eg.db)

# 提取关键列
# Extract relevant columns 
boYingult <- boYingult[,c(2,4)]
colnames(boYingult) <- c("ENTREZID","class")

# 合并基因符号映射
# Merge with symbol mapping 
boYingult <- merge(tmp,boYingult,by = "ENTREZID", all.x = T)
colnames(boYingult)[2] <- "probe"
...
```{r}
tcga.boYingult <- ntp(emat = t(scale(t(expr))),
                    templates = boYingult,
                    doPlot = T,
                    seed = 19991018)
...
annCol <- data.frame(CTNNB1 = ifelse(as.numeric(mut.binary["CTNNB1",]) == 0,"WT","MT"),
                     ...
                     BoYingult = as.character(tcga.boYingult$prediction),
...
annCol$BoYingult <- factor(annCol$BoYingult, levels = c("G12","G3","G56"))
...
annColors[["BoYingult"]] <- c("G12" = "#E8536B","G3" = "#F6B879","G56" = "#DFEBAF")
...
hm <- pheatmap(plotdata,
         ...
         show_rownames = F,
         show_colnames = F,
         annotation_col = annCol[,c("iCluster","Chiang","BoYingult","Hoshida","Lee","CTNNB1","MS","Group")],
...
for (i in c("iCluster","Chiang","BoYingult","Hoshida","Lee","CTNNB1")) {
