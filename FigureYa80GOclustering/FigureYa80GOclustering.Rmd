...
#           这些节点编号对应之前树形图中的内部节点
# Define 4 major classification nodes manually based on phylogenetic tree structure and biological knowledge
#                  These node numbers correspond to internal nodes in the previous tree visualization
node <- c(22,21,27,25)

# 基于指定节点将GO term划分为不同的进化分支组
# Group GO terms into different evolutionary clades based on specified nodes
gtree <- groupClade(tree, .node=node)

# 使用ggtree绘制分组后的树形图，不同分支组用不同颜色区分
# Plot the grouped tree using ggtree, coloring different clades with distinct colors
pbase <- ggtree(gtree, 
                aes(color=group)) # Color branches by group

# 设置注释文本大小和位置参数
# Set parameters for annotation text size and position
...
# Construct the complete tree visualization with clade labels and GO term names
pnode <- pbase + 
...
# 在树形图右侧添加热图，展示各组的p.adjust值
#           热图颜色从红色（高值）到白色（低值），表示富集显著性
# Add a heatmap to the right of the tree to show p.adjust values for each group
